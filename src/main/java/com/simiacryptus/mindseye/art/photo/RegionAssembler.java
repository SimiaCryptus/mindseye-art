...
public abstract class RegionAssembler implements Comparator<RegionAssembler.Connection> {
  ...
  public RegionAssembler(SparseMatrixFloat graph, int[] pixelMap, Map<Integer, Integer> assignments, IntFunction<double[]> pixelFunction, IntFunction<double[]> coordFunction, Predicate<Connection> connectionFilter) {
    graph.assertSymmetric();
    this.regionCount = graph.rows;
    this.pixels = pixelMap.length;
    this.connectionFilter = connectionFilter;
    final Region[] rawRegions = IntStream.range(0, graph.rows).mapToObj(row -> new Region(row)).toArray(i -> new Region[i]);
    final Map<Integer, List<Integer>> pixelAssignmentMap = IntStream.range(0, pixelMap.length).mapToObj(x -> x).collect(Collectors.groupingBy(x -> pixelMap[x], Collectors.toList()));
    final List<Region> collect = Arrays.stream(graph.activeRows()).parallel().mapToObj(row -> {
      ...
      if (null != assignment) region.marks.add(assignment);
      if (cols.length != 0) {
        region.connections_addAll(IntStream.range(0, cols.length)
            .filter(x -> Double.isFinite(vals[x]) && vals[x] != 0)
            .mapToObj(x -> new Connection(region, rawRegions[cols[x]], vals[x]))
    ...
    assert this.regions.stream().flatMap(x -> x.connections.values().stream()).allMatch(connection -> {
      ...
      if (null == reciprical) return false;
      return Math.abs(connection.value - reciprical.value) < 1e-4;
    });
  }

  @NotNull
  public static RegionAssembler wrap(SparseMatrixFloat graph, int[] pixelMap,
                                     Function<Connection, Double> extractor,
                                     final Tensor content,
                                     final RasterTopology topology,
    ...
    final IntFunction<double[]> coordFunction = p -> Arrays.stream(topology.getCoordsFromIndex(p)).mapToDouble(x -> x).toArray();
    final Predicate<Connection> connectionFilter = connection -> connection.to != connection.from && extractor.apply(connection) < Double.POSITIVE_INFINITY;
  ...
  public @NotNull
  static RegionAssembler volumeEntropy(SparseMatrixFloat graph, int[] pixelMap, Tensor content, RasterTopology topology) {
    return wrap(graph, pixelMap, new Function<Connection, Double>() {
      ...
      public Double apply(Connection entry) {
        ...
        final double smallness = reduce(
            entry.to.pixels.size(),
            entry.from.pixels.size()
        );
  ...
  static RegionAssembler epidemic(SparseMatrixFloat graph, int[] pixelMap, Tensor content, RasterTopology topology, Map<Integer, Integer> assignments) {
    return wrap(graph, pixelMap, new Function<Connection, Double>() {
      ...
      public Double apply(Connection entry) {
        ...
        if (to_marks.length == 0 && from_marks.length == 0) return Double.POSITIVE_INFINITY;
        if (to_marks.length > 0 && from_marks.length > 0) {
          if (to_marks[0] == from_marks[0]) return -Double.POSITIVE_INFINITY;
  ...
  public @NotNull
  static RegionAssembler volume5D(SparseMatrixFloat graph, int[] pixelMap, Tensor content, RasterTopology topology) {
    ...
    return wrap(graph, pixelMap, new Function<Connection, Double>() {
      ...
      public Double apply(Connection entry) {
        ...
        final double log_vol_union = logVol(
            union(entry.to.colorStats, entry.from.colorStats),
        ...
        final double split_entropy = to_pixels * log((double) to_pixels / (to_pixels + from_pixels)) + from_pixels * log((double) from_pixels / (to_pixels + from_pixels));
      ...
      public double volumeExtrema(double minVol, DoubleVectorStatistics union) {
        return Arrays.stream(union.firstOrder)
            ...
            .map(x -> Math.abs(x) < minVol ? minVol : x)
            .reduce((a, b) -> a * b)
            .orElse(0);
      }
      ...
      public double volumeStdDev(double minVol, DoubleVectorStatistics stats) {
        ...
        return IntStream.range(0, firstOrder.length)
            .mapToDouble(i -> Math.pow(Math.abs(secondOrder[i].getAverage() - Math.pow(firstOrder[i].getAverage(), 2)), 0.5))
            .map(x -> Math.abs(x) < minVol ? minVol : x)
            .reduce((a, b) -> a * b)
            .orElse(0);
      }
  ...
  public static int[] reduce(SparseMatrixFloat graph, int targetCount, final int[] sizes, Tensor content, RasterTopology topology) {
    return wrap(graph, sizes, (Connection entry) -> {
          return null == entry ? Double.POSITIVE_INFINITY : entry.value;
        }, content, topology, new HashMap<Integer, Integer>()
    ).reduceTo(targetCount).getProjection();
  }
  ...
  public RegionAssembler reduceTo(int count) {
    while (regions.parallelStream().map(Region::connections_stream).filter(stream -> stream
        .filter(connectionFilter).count() > 0).limit(count + 1).count() > count) {
      final int limit = Math.max(1, (regions.size() - count) / 100);
      List<Connection> first = regions.parallelStream()
          .flatMap(Region::connections_stream_parallel)
  ...
  public class Region {
    ...
    private void union(Region other) {
      final List<Connection> newConnections = Stream.concat(
          other.connections_stream(),
          connections_stream()
          ...
          .map(c -> new Connection(this, c.to, c.value))
          .collect(Collectors.toList());
      ...
      newConnections.stream().allMatch((v) -> {
        ...
        final List<Connection> connectionsToRemove = thirdNode.connections_stream().filter(x -> x.to == other || x.to == this).collect(Collectors.toList());
        //assert Math.abs(connectionsToRemove.stream().mapToDouble(x -> x.value).sum() - v.value) < 1e-3;
        assertEmpty(connectionsToRemove.stream().filter(o -> !thirdNode.connections_remove(o)).collect(Collectors.toList()));
